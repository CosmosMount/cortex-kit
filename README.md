...
Run **Cortex Kit: Sample / CSV** from the Command Palette, or click the record icon in Live Watch / Plots. The bottom panel has separate **Plot** and **Sample** tabs. **Plot** shows live curves and **Sample** handles recording and CSV import. The command focuses the Sample tab; each tab has its own panel container.
