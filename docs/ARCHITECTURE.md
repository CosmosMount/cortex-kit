...
Live Watch and Plot keep separate persisted variable selections. The extension merges their raw dependencies into one deduplicated Worker subscription, then routes the newest samples to the native Live Watch tree and only chart-selected channels to the Plot Webview. A variable selected in both places is read once. With no plotted variables, Live Watch uses its lower configured sampling rate. When Plot is active, the shared hardware stream runs at the higher requested rate while the Live Watch tree throttles its UI refresh.
