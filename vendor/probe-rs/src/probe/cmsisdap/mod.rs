...
impl CmsisDap {
    ...
    fn process_batch(&mut self) -> Result<Option<u32>, ArmError> {
        ...
        Ok(self.process_commands(batch)?.last().copied().flatten())
    }

    fn process_commands(&mut self, batch: Vec<BatchCommand>) -> Result<Vec<Option<u32>>, ArmError> {
        if batch.is_empty() {
            return Ok(Vec::new());
        }
...
impl RawDapAccess for CmsisDap {
    ...
    fn raw_write_block(
        ...
        if !values.is_empty()
            ...
            && self
                .batch
                .iter()
                .all(|command| matches!(command, BatchCommand::Write(_, _)))
        {
            let capacity = (self.packet_size as usize).saturating_sub(3 + self.batch.len() * 5) / 5;
            let combined = values.len().min(capacity);
            if combined > 0 {
                let mut batch = std::mem::take(&mut self.batch);
                batch.extend(
                    values[..combined]
                        .iter()
                        .map(|value| BatchCommand::Write(address, *value)),
                );
                self.process_commands(batch)?;
    ...
    fn raw_read_block(
        ...
        // DAP_TransferBlock forces pending CSW/TAR writes into a separate USB
        // command. For a small block, place those writes and all repeated DRW
        // reads in one DAP_Transfer packet instead.
        let request_capacity = (self.packet_size as usize).saturating_sub(3 + self.batch.len() * 5);
        let response_capacity = (self.packet_size as usize).saturating_sub(3) / 4;
        let combined_reads = values.len().min(request_capacity).min(response_capacity);
                ...
                .all(|command| matches!(command, BatchCommand::Write(_, _)))
        {
            ...
            let responses = self.process_commands(batch)?;
            for (target, value) in values[..combined_reads]
                .iter_mut()
                .zip(responses.into_iter().skip(write_count))
            {
                *target = value.ok_or_else(|| {
                    ArmError::Other("CMSIS-DAP read transfer returned no data".into())
                })?;
            }
            if combined_reads == values.len() {
                return Ok(());
            }
            values = &mut values[combined_reads..];
        }
    ...
    fn raw_write_read_block(
        &mut self,
        write_address: RegisterAddress,
        read_address: RegisterAddress,
        write_values: &[u32],
        read_values: &mut [u32],
    ) -> Result<(), ArmError> {
        if write_values.len() != read_values.len() {
            return Err(ArmError::Other(
                "write/read block lengths must match".into(),
            ));
        }

        let mut completed = 0;
        while completed < write_values.len() {
            if !self
                .batch
                .iter()
                .all(|command| matches!(command, BatchCommand::Write(_, _)))
            ...
            let request_capacity = (self.packet_size as usize).saturating_sub(3 + pending * 5) / 6;
            let response_capacity = (self.packet_size as usize).saturating_sub(3) / 4;
            let count = (write_values.len() - completed)
                .min(request_capacity)
                .min(response_capacity);
            if count == 0 {
                self.process_batch()?;
                continue;
            }
            ...
            let mut batch = std::mem::take(&mut self.batch);
            for value in &write_values[completed..completed + count] {
                batch.push(BatchCommand::Write(write_address, *value));
                batch.push(BatchCommand::Read(read_address));
            }
            let responses = self.process_commands(batch)?;
            let reads = responses.into_iter().skip(pending + 1).step_by(2);
            for (target, value) in read_values[completed..completed + count]
                .iter_mut()
                .zip(reads)
            {
                *target = value.ok_or_else(|| {
                    ArmError::Other("CMSIS-DAP paired read returned no data".into())
                })?;
            }
            completed += count;
        }
