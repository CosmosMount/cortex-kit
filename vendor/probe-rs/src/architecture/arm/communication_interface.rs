...
impl DapAccess for ArmCommunicationInterface {
    ...
    fn read_raw_ap_memory_32(
        ...
        self.select_ap_and_ap_bank(ap, TAR::ADDRESS)?;
        self.probe_mut().raw_write_read_block(
            RegisterAddress::ApRegister((TAR::ADDRESS & 0xff) as u8),
            RegisterAddress::ApRegister((DRW::ADDRESS & 0xff) as u8),
            addresses,
            values,
        )
    }
