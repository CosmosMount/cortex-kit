    ...
    AP: DapAccess,
{
    ...
    fn read_32_scattered(&mut self, addresses: &[u64], data: &mut [u32]) -> Result<(), ArmError> {
        assert_eq!(addresses.len(), data.len());
        if let Some(address) = addresses.iter().find(|address| !address.is_multiple_of(4)) {
            return Err(ArmError::alignment_error(*address, 4));
        }
        ...
        if addresses
            .iter()
            .any(|address| u32::try_from(*address).is_err())
        {
            for (address, value) in addresses.iter().zip(data) {
                self.read_32(*address, std::slice::from_mut(value))?;
            }
            return Ok(());
        }
        ...
        self.memory_ap
            .try_set_datasize(self.interface, DataSize::U32)?;
        self.interface
            .read_raw_ap_memory_32(self.memory_ap.ap_address(), &addresses, data)
    }
