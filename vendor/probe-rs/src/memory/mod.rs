    ...
    ERR: std::error::Error + From<InvalidDataLengthError> + From<MemoryNotAlignedError>,
{
    ...
    /// Read unrelated 32-bit words. Implementations may batch the accesses.
    fn read_32_scattered(&mut self, addresses: &[u64], data: &mut [u32]) -> Result<(), ERR> {
    ...
    Error: From<<T as CoreMemoryInterface>::ErrorType>,
{
    ...
    fn read_32_scattered(&mut self, addresses: &[u64], data: &mut [u32]) -> Result<(), Error> {
        self.memory_mut()
            .read_32_scattered(addresses, data)
            .map_err(Error::from)
    }
